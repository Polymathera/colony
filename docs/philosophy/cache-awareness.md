...
Most agent frameworks treat inference infrastructure as a black box: send a prompt, get a response. Colony treats it as the central architectural concern, because when context spans billions of tokens across a GPU cluster, **cache misses dominate execution time**.

## The Key Insight

> **Cache awareness is NOT a property of primitives -- it is an emergent property of the LLM planner composing primitives.**

This distinction matters. You cannot make individual tools or actions "cache-aware" in isolation. The most efficient strategy for managing KV cache pages depends on the *data being analyzed at runtime* -- the structure of the context, the relationships between pages, the current state of the reasoning process. Only the planner, which sees the full picture, can make cache-aware decisions.
...
Every plan in Colony includes comprehensive cache context. This is not metadata -- it is a core part of the plan that the planner reasons about:
...
When an agent is ready to execute a plan, Colony does not assign it to an arbitrary node. The **PageAffinityRouter** selects the node where the most relevant pages are already cached:
