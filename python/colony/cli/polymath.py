...
def deploy(
    ...
    """
    console.print(f"Deploying version {version} to {environment}")
...
def run_server(name: str):
    print(f"Running server {name}")
    import api_server
    api_server.app.run(debug=True, host='127.0.0.1', port=8001)



@app.command()
def check_db_health(name: str, formal: bool = False):
    if formal:
...
if __name__ == "__main__":
    console.print("Starting Polymathera CLI")
    app()
