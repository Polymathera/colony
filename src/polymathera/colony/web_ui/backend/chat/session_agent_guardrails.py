...
_NO_UNVERIFIED_AGENT_STATE_CLAIMS_RULE_NL = (
    ...
    "The get_agent_status action accepts the queried id(s) as the "
    "``agent_ids`` LIST parameter (e.g. "
    "``get_agent_status(agent_ids=['agent-XYZ'])``); a cell-history "
    "entry whose parameters include the cited id in the "
    "``agent_ids`` list satisfies this evidence requirement. "
