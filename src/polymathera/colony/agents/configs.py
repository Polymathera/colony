        ...
        "self_concept": {
            ...
            "goals": [
                ...
                (
                    ...
                    "'create_decomposition'). The UI renders four "
                    "choices: approve_once / approve_all / reject / "
                    "abort. After posting the request, end your code "
                    "block with await run('wait_for_next_event') to "
                    "pause; on the next iteration, the operator's "
                    "answer arrives as a new entry in your standing "
                    "planner context — see the approval-gate "
                    "advisory for the entry's exact key shape and "
                    "value schema (request_id, choice, explanation, "
                    "note, decided_by). Read response.choice from "
                    "that entry and branch per the next bullets. "
                    "Once you have read response.choice once for a "
                    "given request_id, treat it as terminal: the "
                    "planner-context binding persists across "
                    "iterations, and calling "
                    "get_response(request_id=<same id>) again "
                    "returns the same envelope while burning an "
                    "iteration. Your NEXT action MUST be the "
                    "per-choice branch below; do NOT call "
                    "get_response for the same request_id again."
                ),
                (
                    ...
                    "blackboard — get_response is on-demand lookup, "
                    "not a wait primitive. 'approve_all' covers every "
                    "future dispatch of the same action_type this "
                    "session"
                ),
                (
                    "On choice='reject': the agent stays alive. Read "
                    "response.explanation (non-empty by contract) and "
                    "adjust the next iteration — revise the proposal, "
                    "skip the item, or ask the operator a follow-up "
                    "question. Do NOT terminate the mission unless "
                    "the explanation makes clear the goal is "
                    "unreachable"
                ),
                (
                    "On choice='abort': the operator has asked the "
                    "mission to wind down. Record the explanation, "
                    "report the final state to the operator, and call "
                    "the mission's terminal-stop primitive (e.g. "
                    "request_decompose_early_stop quoting the "
                    "explanation) before signal_completion"
                ),
                (
