...
class DeferredClosure(Generic[R], ABC):
    ...
    at cloudpickle-dumps time if a closure can't be shipped.

    Provenance snapshot: the capability instance is read for its
    :attr:`spec.name` (tool name) + the agent's current run-id, then
    discarded. The closure carries these as plain strings so it can
    build provenance metadata without reaching back to a (no longer
